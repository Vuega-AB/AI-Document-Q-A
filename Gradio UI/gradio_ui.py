...
# Import backend functions that are directly used by Gradio UI definitions
# Note: BACKEND_INITIAL_LOAD_MSG, AVAILABLE_MODELS_NAMES, MODEL_NAME_TO_ID_MAP
# are now initialized in backend.py and will be accessed from there.
from backend import (
    ...
    AVAILABLE_MODELS_NAMES, # Loaded from backend
    MODEL_NAME_TO_ID_MAP,   # Loaded from backend
    BACKEND_INITIAL_LOAD_MSG, # Loaded from backend
)
...
# This function creates and returns the Gradio Blocks instance
def create_gradio_app():
    with gr.Blocks(
        ...
        selected_db_state = gr.State(value="Dropbox") # Default DB
        initial_selected_model_id = []
        
        # Check if backend variables are populated before accessing
        if AVAILABLE_MODELS_NAMES and MODEL_NAME_TO_ID_MAP and AVAILABLE_MODELS_NAMES[0] in MODEL_NAME_TO_ID_MAP:
        ...
        with gr.Row(equal_height=False):
            with gr.Column(scale=2, min_width=420, elem_classes=["controls-column"]):
                ...
                with gr.Accordion("Database Backend", open=True):
                    ...
                    # BACKEND_INITIAL_LOAD_MSG is now imported from backend.py
                    db_status = gr.Textbox(label="DB Status", interactive=False, value=BACKEND_INITIAL_LOAD_MSG)


                with gr.Tabs():
                    with gr.TabItem("🧠 Config"):
                        # Ensure AVAILABLE_MODELS_NAMES is populated before using
                        model_selector_default_value = []
                        if AVAILABLE_MODELS_NAMES:
                             model_selector_default_value = [AVAILABLE_MODELS_NAMES[0]] if AVAILABLE_MODELS_NAMES[0] in MODEL_NAME_TO_ID_MAP else []
            ...
            with gr.Column(scale=3):
                ...
                chatbot = gr.Chatbot(label="IntelLaw Chatbot", height=600, show_copy_button=True, bubble_full_width=False,type="messages" )
...
# The old `if __name__ == '__main__': demo.launch()` is removed.
# Flask will import `create_gradio_app` and manage the app.